  ...
  return (
    <section id="producto" className="bg-slate-950 py-20 text-white">
      <div className="mx-auto grid max-w-7xl items-center gap-10 px-4 sm:px-6 lg:grid-cols-2">
        <Reveal className="relative mx-auto max-w-sm rounded-xl bg-white p-8 shadow-2xl">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            src={product.imagen}
            alt={`Bolsa de detergente ${product.nombre}`}
            className="max-h-[520px] w-auto object-contain drop-shadow-2xl"
          />
        </Reveal>
        <Reveal>
          <Card data-motion-card className="border-2 border-cyan-300 bg-white text-slate-950">
            <CardContent className="space-y-6 p-8">
              ...
              <div>
                ...
                <p className="mt-2 text-lg text-slate-600">
                  {product.presentacion} · {product.peso}
                </p>
              </div>
